...
int main(void) {
    u8 dev_Addr = 0x51 << 1;    // 设备写地址 0xA2
    u8 mem_Addr = 0x02;         // 寄存器地址: 从秒钟寄存器开始读
    u8 seconds,minutes,hours,days,weekdays,months,C;
    u16 years;
    // 读写数据数组
    u8 p[NUMBER] = {0};
    ...
    ******************************************/
    ...
    I2C_WriteNbyte(dev_Addr, 0x02, p, NUMBER);
    
    while (1) {
        I2C_ReadNbyte(dev_Addr, 0x02, p, NUMBER);
