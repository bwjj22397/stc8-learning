"use strict";
/* 本文件由 tests/build-engine.js 覆盖到 engine-src（基于 JSCPP 2.0.9 lib/includes/iostream.js 修补）：
 * 1. cin >> 整数：原正则可匹配空串且分支未锚定，改为 /^[-+]?[0-9]+/
 * 2. cin >> 浮点：原正则 ^ 只作用于第一个分支，负整数输入会把负号留在缓冲区、按正数读入
 *    （cin>>double 读 -5 得 5 的根因），重写为完整数字文法
 */
                    ...
                    default(rt, _cin, t) {
                        ...
                        switch (t.t.name) {
                            ...
                            case "unsigned long long int":
                                b = _skipSpace(b);
                                r = _read(rt, /^[-+]?[0-9]+/, b, t.t);
                                v = parseInt(r[0], 10);
                                break;
                            case "float":
                            case "double":
                                b = _skipSpace(b);
                                r = _read(rt, /^[-+]?(?:[0-9]+\.[0-9]*|\.[0-9]+|[0-9]+)(?:[eE][-+]?[0-9]+)?/, b, t.t);
                                v = parseFloat(r[0]);
                                break;
                            case "bool":
                                b = _skipSpace(b);
