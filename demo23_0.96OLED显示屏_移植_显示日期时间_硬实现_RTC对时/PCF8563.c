...
void PCF8563_enable_Alarm(u8 enable) {
    ...
    I2C_ReadNbyte(PCF8563_ADDR, 0x01, &enAlarm, 1);
    ...
    I2C_WriteNbyte(PCF8563_ADDR, 0x01, &enAlarm, 1);
}
...
void PCF8563_set_Timer(u8 frequency,u8 countdown) {
    ...
    I2C_WriteNbyte(PCF8563_ADDR, TIMER_HZREG, &p, 1);
    
    // 4.设置Timer计数值n
    p = countdown;
    I2C_WriteNbyte(PCF8563_ADDR, TIMER_REG, &p, 1);
}
...
void PCF8563_enable_Timer(u8 enable) {
    ...
    I2C_ReadNbyte(PCF8563_ADDR, 0x01, &enTimer, 1);
    ...
    I2C_WriteNbyte(PCF8563_ADDR, 0x01, &enTimer, 1);
}

void exti_int3_call(void) {
    u8 reg_Status;
    // 读取cs2控制寄存器的值, 查看AF和TF标记
    I2C_ReadNbyte(PCF8563_ADDR, 0x01, &reg_Status, 1);
    ...
    // 一次性把修改后的信息写回0x01寄存器
    I2C_WriteNbyte(PCF8563_ADDR, 0x01, &reg_Status, 1);
}